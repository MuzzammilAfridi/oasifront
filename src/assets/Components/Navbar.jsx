...
function Navbar({isAdmin, setIsprofile, open, setOpen, count1 }) {
  ...
  useEffect(() => {
    const fetchCart = async () => {
      if (!userId) return; // Ensure userId is available before making a request
...
<div className="flex gap-3 ">
   ...
   <div className=" flex items-center space-x-4">
          ...
          <button className="flex items-center relative text-gray-700 hover:text-blue-500">
            <div className="bg-red-700 text-[10px]  text-white flex items-center justify-center rounded-full w-3 absolute top-[2px] left-[10px] h-3">{cart && cart.products.length}</div>
